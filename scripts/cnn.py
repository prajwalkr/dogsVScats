...
def init_model(preload=None, use_vgg=False, use_resnet=True):
	if use_resnet:
		model = ResNet.build_resnet_18((channels, img_width, img_height), 2)
		if preload:
			model.load_weights(preload)
		return model
...
def random_crop(img):
	rand_side = randint(img_width + 1, 340)
...
def standardized(gen, training=False, vgg=False):
	...
	while 1:
		...
		for i in xrange(len(X)):
			x[i] = random_crop(X[i]) if training else resize(X[i], img_height, img_width)
			if vgg:
				x[i] = x[i][::-1]
...
def DataGen():
	side_len = 360
	train_datagen = ImageDataGenerator(horizontal_flip=True, channel_shift_range=2.)
...
def runner(model, epochs):
	...
	model.compile(optimizer=SGD(initial_LR, momentum=0.9), loss='categorical_crossentropy')
